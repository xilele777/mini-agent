...
async function fixture(t: test.TestContext, text = 'before\r\nkeep\r\n') {
  ...
  return { dir, path, text, args: { path, expected_sha256: fingerprint(text), old_text: 'before', new_text: 'after' } }
}
...
test('读取分页与空文件均提供整文件指纹', async t => {
  ...
  assert.match(await read.execute({ path: f.path, offset: 1, limit: 1 }), new RegExp(fingerprint('')))
})
...
test('批准后出现同名文件，不覆盖；取消不提交', async t => {
  ...
  await assert.rejects(async () => edit.execute({ signal: AbortSignal.abort() }), { name: 'AbortError' })
  assert.equal(await readFile(f.path, 'utf8'), f.text)
})

test('文件工具统一拒绝项目外、敏感路径和无法解析路径，搜索允许项目根', async () => {
  for (const path of ['../outside.txt', '.env', '.git/new.txt', '.mini-agent/snapshot.json', 'node_modules/new.txt']) {
...
test('实际项目内 junction 别名不能绕过敏感目录或写入链接规则', async t => {
  ...
  await assert.rejects(writeFileTool.prepare!({ path: join(alias, 'new.txt'), content: 'x' }), /链接/)
})

test('拒绝二进制与超限文件；失败不留下临时文件', async t => {
  const f = await fixture(t, '\0binary')
  await assert.rejects(editFileTool.prepare!(f.args), /UTF-8/)
  await assert.rejects(writeFileTool.prepare!({ path: join(f.dir, 'large'), content: 'x'.repeat(1024 * 1024 + 1) }), /上限/)
  assert.deepEqual(await readdir(f.dir), ['file.txt'])
})

test('Windows 盘符大小写不同仍能完成路径检查', { skip: process.platform !== 'win32' }, async t => {
  const f = await fixture(t)
...
function response(args: unknown): AsyncIterable<ChatCompletionChunk> {
  return (async function* () {
    yield {
      id: 'edit', object: 'chat.completion.chunk', created: 0, model: 'test',
      choices: [{ index: 0, delta: { tool_calls: [
        { index: 0, id: 'edit', type: 'function', function: { name: 'edit_file', arguments: JSON.stringify(args) } },
...
test('主轮用同一候选预览和提交；拒绝及批准等待取消均无写入', async t => {
  for (const mode of ['allow', 'deny', 'cancel', 'conflict', 'invalid'] as const) {
    ...
    const result = await runTurn([{ role: 'user', content: 'edit', startsTurn: true }], {
      registry: createToolRegistry([editFileTool, finishTaskTool]), contextBudget: testConfig.contextBudget,
      maxIterations: 1, signal: controller.signal, write: () => {}, log: () => {},
      createStream: async () => response(mode === 'invalid' ? { ...f.args, old_text: 'missing' } : f.args),
    ...
    assert.equal(await readFile(f.path, 'utf8'), mode === 'allow' ? 'after\r\nkeep\r\n' : mode === 'conflict' ? 'external' : f.text)
    ...
    if (mode === 'deny' || mode === 'cancel' || mode === 'invalid') {
      assert.ok(events.some(e => e.type === 'action' && e.call.id === 'edit' && e.state === 'not_executed'))
      assert.ok(!events.some(e => e.type === 'action' && e.call.id === 'edit' && e.state === 'started'))
    }
  }
})
