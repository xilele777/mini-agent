...
export function createReadFileTool(maxReadBytes: number): Tool<z.infer<typeof readParams>> {
  return {
    ...
    schema: readParams,
    description:
      '按行读取项目内文本文件，返回整文件 SHA-256，供 edit_file 校验基准版本。未到末尾时用 offset 翻页。拒绝项目外、敏感文件和超限文件。修改前必须先读取。内容保留原始换行。',
    execute: async ({ path, offset, limit }, context) => {
      ...
      try {
        ...
        if (all.includes('\0') || !Buffer.from(all).equals(data))
          return '错误:只支持无 NUL 的 UTF-8 文本'
