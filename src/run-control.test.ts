        ...
        delta: names.length
          ? {
              tool_calls: names.map((name, index) => ({
                ...
                function: {
                  name,
                  arguments: name === 'delegate_task' ? '{"task":"inspect"}' : '{}',
                },
              })),
            }
          : { content: text },
...
async function* stream(...chunks: ChatCompletionChunk[]) {
  yield* chunks
}
const request = { messages: [], tools: [], maxOutputTokens: 128 }
const busy = () =>
  new OpenAI.APIError(503, { message: 'SECRET remote body' }, 'SECRET', new Headers())
const done: Tool = {
  name: 'done',
  description: 'done',
  schema: z.object({}),
...
function options(createStream: CreateTurnStream, tools: Tool[] = [done]): RunTurnOptions {
  return {
    ...
    runLimits: { ...DEFAULT_RUN_LIMITS, retryBaseMs: 1 },
    log: () => {},
    write: () => {},
  }
}
...
test('每次传输尝试独立扣总次数，503 有限重试并结算实际与未知 usage', async () => {
  ...
  const control = new RunControl({ ...DEFAULT_RUN_LIMITS, retryBaseMs: 1 }, undefined, (e) =>
    events.push(e)
  )
  let calls = 0
  try {
    await collectResponse(
      await control.stream(async () => {
      ...
      async () =>
        collectResponse(
          await control.stream(
            ...
            { ...request, scope: 'subagent', taskId: randomUUID() }
          ),
          () => {}
        ),
      RunBudgetError
...
test('token 预留不足时零请求；usage 缺失不记零，实际超额阻止执行工具', async () => {
  ...
  const o = options(async () => {
    calls++
    return stream(chunk(['done'], '', 300_000))
  }, [
    {
      ...done,
      execute: () => {
...
test('取消退避立即停止，不发送下一次请求', async () => {
  ...
  const control = new RunControl(
    { ...DEFAULT_RUN_LIMITS, retryBaseMs: 1000 },
    parent.signal,
    (e) => {
...
test('批准取消归 cancelled；started 检查点期间取消可正常保存 uncertain', async () => {
  for (const phase of ['approval', 'started']) {
    ...
    o.approve = async () => {
      if (phase === 'approval') parent.abort()
      return true
    }
    o.checkpoint = async (e) => {
      events.push(e)
    ...
    assert.ok(
      events.some(
        (e) =>
          e.type === 'action' && e.state === (phase === 'started' ? 'uncertain' : 'not_executed')
      )
    )
  }
})
...
test('runtime 委派与主轮共用次数和 taskId，子预算不足停止整个主轮', async () => {
  ...
  const runtime = createRuntime(
    { ...testConfig, runLimits: { ...DEFAULT_RUN_LIMITS, maxRequests: 2 } },
    async () => {
      calls++
...
test('真实 SDK 的 503 重试只有一层，部分 SSE 后失败不重试', async () => {
  for (const partial of [false, true]) {
    ...
    const model = createModelStream(testConfig, async () => {
      ...
      if (partial)
        return new Response(
          new ReadableStream({
            start(controller) {
              controller.enqueue(
                new TextEncoder().encode(`data: ${JSON.stringify(chunk([], 'visible'))}\n\n`)
              )
      ...
      if (calls < 3)
        return new Response('{"error":{"message":"SECRET"}}', {
          status: 503,
...
test('主／子／摘要等待中的 SDK 请求均收到同一个取消信号', async () => {
  for (const scope of ['main', 'subagent', 'summary'] as const) {
    ...
    try {
      await assert.rejects(
        async () => collectResponse(await control.stream(model, { ...request, scope }), () => {}),
        { name: 'AbortError' }
      )
...
test('总时限中止模型等待并归预算耗尽，单次请求超时仍为有限重试失败', async () => {
  for (const overall of [true, false]) {
    let calls = 0
    const model = createModelStream(
      { ...testConfig, requestTimeoutMs: overall ? 1000 : 20 },
      async (_input, init) => {
        calls++
...
test('预取消的文件读取和递归搜索不吞掉取消', async () => {
  ...
  await assert.rejects(
    async () =>
      createReadFileTool(1024).execute({ path: 'README.md', offset: 1, limit: 1 }, { signal }),
    { name: 'AbortError' }
  )
  await assert.rejects(
    async () => grepTool.execute({ path: '.', pattern: 'test', maxResults: 1 }, { signal }),
    { name: 'AbortError' }
  )
})
...
test('JSONL 白名单剔除任意正文，写失败仅警告一次', async () => {
  ...
  try {
    ...
    const trace = createTrace(file, randomUUID(), randomUUID(), () =>
    ...
    let warnings = 0
    const broken = createTrace(
      join(root, 'missing', 'trace.jsonl'),
      randomUUID(),
      randomUUID(),
      () => {
        warnings++
...
test('持久化轨迹按 sessionId/turnId 关联，每个新轮重置预算且恢复不重放', async () => {
  ...
  try {
    ...
    for (let i = 0; i < 2; i++)
      assert.equal((await runSessionTurn(session, 'SECRET request', o)).status, 'completed')
    const rows = (
      await readFile(
        join(root, '.mini-agent', 'sessions', session.snapshot.id, 'trace.jsonl'),
        'utf8'
      )
    )
      .trim()
    ...
    assert.deepEqual(
      rows.filter((e) => e.type === 'turn_end').map((e) => e.requests),
      [1, 1]
    )
    assert.deepEqual(
      [...new Set(rows.map((e) => e.turnId))],
      session.snapshot.turns.map((t) => t.id)
    )
    await session.close()
    const reopened = await openSession(root, session.snapshot.id)
    try {
      assert.equal(reopened.snapshot.turns.at(-1)?.status, 'completed')
    } finally {
  ...
  try {
    await session.update((s) => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        s.messages.push(
          { role: 'user', content: 'x'.repeat(900), startsTurn: true },
    ...
    const o = options(async () => {
      calls++
      throw busy()
    }, [{ ...done, description: 'd'.repeat(2300) }])
    ...
    assert.equal(calls, 2)
    assert.deepEqual(
      events.filter((e) => e.type === 'request_start').map((e) => e.scope),
      ['summary', 'summary']
    )
  ...
  for (const mode of ['input', 'approval', 'ask_user']) {
    ...
    const output = await new Promise<string>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        child.kill()
        reject(new Error('readline cancellation timed out'))
      }, 5000)
...
test('共享预算配置拒绝非法值，允许显式禁用重试', () => {
  ...
  for (const field of [
    'MINI_AGENT_MAX_REQUESTS',
    'MINI_AGENT_MAX_TOTAL_TOKENS',
    'MINI_AGENT_TURN_TIMEOUT_MS',
    'MINI_AGENT_RETRY_BASE_MS',
  ]) {
