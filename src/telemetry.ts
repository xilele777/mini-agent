...
export const eventSchema = z.discriminatedUnion('type', [
  ...
  z.object({
    ...
    tokens: number,
    accounting: z.enum(['actual', 'estimated']),
    elapsedMs: number,
  }),
  z.object({
  ...
  z.object({
    ...
    action: number,
    tool: toolName,
    state: z.enum(['started', 'returned', 'not_executed', 'uncertain']),
  }),
  z.object({
    type: z.literal('approval'),
    action: number,
    state: z.enum(['waiting', 'allowed', 'denied']),
  }),
...
export function renderEvent(event: RuntimeEvent): string | undefined {
  switch (event.type) {
    case 'request_start':
      return `  [run] ${event.scope} 请求 #${event.request}，预留 ${event.reservedTokens} tokens`
    case 'retry':
      return `  [retry] ${event.scope} 第 ${event.attempt} 次重试，等待 ${event.delayMs}ms`
    case 'tool':
      return event.state === 'started'
        ? `  [tool] ${event.scope} ${event.tool} #${event.action} 开始`
        : undefined
    case 'turn_end':
      return `  [run] ${event.status} | 请求 ${event.requests} | 工具 ${event.toolCalls} | tokens ${event.chargedTokens}（已报告 ${event.actualTokens}，估算请求 ${event.estimatedRequests}）| ${event.elapsedMs}ms`
    default:
      return undefined
  }
}
