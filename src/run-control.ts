...
export const DEFAULT_RUN_LIMITS: RunLimits = {
  maxRequests: 24, maxTokens: 200_000, timeoutMs: 600_000,
  maxRetries: 2, retryBaseMs: 500,
}
export class RunBudgetError extends ContextBudgetError {
  constructor(message: string) { super(message); this.name = 'RunBudgetError' }
}
...
function validUsage(usage: CompletionUsage | null | undefined): usage is CompletionUsage {
  return !!usage && [usage.prompt_tokens, usage.completion_tokens, usage.total_tokens]
...
export class RunControl {
  ...
  constructor(
    ...
    for (const [key, value] of Object.entries(limits)) {
      if (!Number.isSafeInteger(value) || value < (key === 'maxRetries' ? 0 : 1)) throw new Error('运行预算必须是有效整数')
    }
  ...
  private expire() { this.controller.abort(new RunBudgetError('本轮总时间预算已耗尽（包含批准等待）')) }
  check() {
    if (performance.now() - this.started >= this.limits.timeoutMs && !this.signal.aborted) this.expire()
  ...
  get remainingRequests() { return Math.max(0, this.limits.maxRequests - this.requests) }
  get stats() {
    return { requests: this.requests, chargedTokens: this.chargedTokens, actualTokens: this.actualTokens,
      estimatedRequests: this.estimatedRequests, elapsedMs: Math.round(performance.now() - this.started), toolCalls: this.toolCalls }
  }
  end(status: TurnStatus) { this.emit({ type: 'turn_end', status, ...this.stats }) }
  dispose() { clearTimeout(this.timer); this.parent?.removeEventListener('abort', this.abort) }
  ...
  stream(create: CreateTurnStream, request: TurnRequest): ReturnType<CreateTurnStream> {
    ...
    async function* consume() {
      for (let attempt = 0; ; attempt++) {
        self.check()
        const reserve = estimateInputTokens(request.messages, request.tools) + request.maxOutputTokens
        if (!self.remainingRequests) throw new RunBudgetError('本轮总请求次数预算已耗尽')
        if (self.chargedTokens + reserve > self.limits.maxTokens) throw new RunBudgetError('本轮累计 token 预算不足以预留本次请求')
        const requestNumber = ++self.requests
        const requestStarted = performance.now()
        self.chargedTokens += reserve
        self.estimatedRequests++
        self.emit({ type: 'request_start', scope, taskId, request: requestNumber, reservedTokens: reserve })
        let seen = false
        ...
        try {
          ...
          self.emit({ type: 'request_end', scope, taskId, request: requestNumber,
            outcome: succeeded ? 'returned' : 'failed', tokens: usage?.total_tokens ?? reserve,
            accounting: usage ? 'actual' : 'estimated', elapsedMs: Math.round(performance.now() - requestStarted) })
        }
        self.check()
        if (succeeded) {
          if (self.chargedTokens > self.limits.maxTokens) throw new RunBudgetError('服务端 usage 超过本轮 token 预算，已停止后续动作')
          return
        }
