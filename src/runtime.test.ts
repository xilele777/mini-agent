...
test('HTML 页面和普通文本不能冒充工具探针成功', async () => {
  for (const fetcher of [
    async () => new Response('<html>home</html>', {
      headers: { 'content-type': 'text/html' },
    }),
    async () => response(false),
  ]) {
    assert.equal(
      (await checkModel(testConfig, fetcher)).ok,
      false
    )
  }
})
...
test('收到响应头后流停滞仍会超时', async () => {
  const result = await checkModel(
    ...
    async (_input, init) => new Response(
      new ReadableStream({
        start(controller) {
...
test('配置的次数、读取上限和 shell 进入实际依赖', async () => {
  ...
  for (const registry of [
    runtime.turnOptions.registry,
    runtime.subagentOptions.registry,
  ]) {
    const call = registry.prepareCall(
      'read_file',
      '{"path":"src/config.ts"}'
    )
  ...
  const call = runtime.turnOptions.registry.prepareCall(
    'run_bash',
    '{"command":"pwd"}'
  )
...
test('委派使用组装好的子 Agent 预算和模型入口', async () => {
  ...
  const runtime = createRuntime(
    { ...testConfig, subagentMaxIterations: 1 },
    async (request) => {
      requests++
...
test('不可用 shell 给出固定诊断，离线 CLI 不探测模型', async () => {
  ...
  const child = spawnSync(process.execPath, [
    '--import',
    'tsx',
    fileURLToPath(new URL('./doctor-cli.ts', import.meta.url)),
  ], {
    encoding: 'utf8',
    timeout: 10_000,
    windowsHide: true,
