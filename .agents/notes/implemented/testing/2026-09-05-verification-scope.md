...
项目使用 node:test、assert 和现有 tsx 执行最小回归。纯函数测试检查截断、重复调用和历史转换；路径及文件工具测试检查部分拒绝与放行情形。npm test 当前运行三个测试文件，共二十一条。
...
回归可在本地快速重复，代价是覆盖有限。当前缺少批准缓存、todo 崩溃与并发、shell 中断、主循环同批结束配对等专门自动化用例。
