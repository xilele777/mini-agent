...
/** Windows 上按 MINI_AGENT_SHELL、PATH 中的 git.exe、默认安装目录顺序寻找 Git Bash。 */
function findGitBash() {
  if (process.env.MINI_AGENT_SHELL) return process.env.MINI_AGENT_SHELL
  const candidates = []
  ...
  assert.ok(found, '未找到 Git Bash；请设置 MINI_AGENT_SHELL 指向 bash.exe')
  return found
}
...
try {
  ...
  for (const name of files) {
    assert.ok(
      /^(dist\/(?:tools\/)?[\w-]+\.js|bin\/mini-agent\.mjs|package\.json|README\.md|LICENSE|CHANGELOG\.md|\.env\.example)$/.test(
        name
      ),
      `意外打包文件：${name}`
    )
  ...
  for (const name of ['dist/agent.js', 'bin/mini-agent.mjs', '.env.example'])
  ...
  await writeFile(
    ...
    JSON.stringify({ name: 'delivery-consumer', private: true })
  )
  ...
  const invoke = (args, overrides = {}, expected = 0) =>
    run(process.execPath, [entry, ...args], project, overrides, expected)
  ...
  if (process.platform === 'win32') {
    ...
    assert.match(
      run(join(consumer, 'node_modules/.bin/mini-agent'), ['--help'], project),
      /--resume/
    )
  }
  const shell = process.platform === 'win32' ? findGitBash() : '/bin/sh'
  const configured = {
    OPENAI_API_KEY: 'delivery-test-only',
  ...
  console.log(
    '[OK] 仅生产依赖安装：真实 npm shim、帮助、版本、退出码、离线 doctor、新建与恢复会话通过。'
  )
} finally {
  const resolved = await realpath(work)
  const rel = relative(base, resolved)
  assert.ok(
    rel.startsWith('mini-agent-delivery-') &&
      !isAbsolute(rel) &&
      !rel.includes('/') &&
      !rel.includes('\\')
  )
