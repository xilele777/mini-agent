...
export function buildDataset(files) {
  ...
  for (const file of [...files].sort((a, b) => compareText(a.path, b.path))) {
    ...
    if (
      parts.length !== 3 ||
      !LIFECYCLES.includes(parts[0]) ||
      !CLASSES.includes(parts[1]) ||
  ...
  for (const [id, file] of selected) {
    ...
    if (lifecycle !== 'archived' && !new RegExp('^' + lifecycle + '(?:$|\\s)').test(status))
      throw new Error('笔记状态与目录不一致：' + file.path)
    const sections = splitSections(rawBody)
    const section = (...headings) =>
      sections.find((item) => headings.includes(item.heading))?.body || ''
    notes.push({
      id,
...
export function datasetSignature(notes) {
  ...
  return JSON.stringify(
    [...notes]
      ...
      .map((note) => [note.id, note.sourcePath, note.rawBody])
  )
}
...
export function alternativeItems(note) {
  return note.alternatives
    .split(/\n(?=###?\s|\s*[-*+]\s)/)
    .map((text) => text.replace(/^\s*(?:#{2,3}\s+|[-*+]\s+)/, '').trim())
    .filter(Boolean)
}
...
export function countReferences(notes) {
  ...
  for (const note of notes)
    for (const id of note.outLinks) if (result.has(id)) result.set(id, result.get(id) + 1)
  return result
}
...
export function groupNotes(notes, presentation = {}) {
  ...
  for (const cls of CLASSES) {
    const entries = notes
      .filter((note) => !used.has(note.id) && note.cls === cls)
      .sort(compareNotes)
      .map((note) => ({ note, label: note.title }))
...
export async function resolveNotesDirectory(root) {
  ...
  for await (const entry of root.values())
    if (entry.kind === 'directory' && LIFECYCLES.includes(entry.name)) return root
  ...
  async function walk(directory, parts = []) {
    ...
    for (const entry of entries) {
      if (
        ...
        (parts.length === 0
          ? LIFECYCLES.includes(entry.name)
          : parts.length === 1 && CLASSES.includes(entry.name))
      ) {
...
export function createLiveSync({
  read = readNoteDirectory,
  signatureOf = datasetSignature,
